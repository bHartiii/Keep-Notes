...
class AuthenticationAPITest(TestCase):
    ...
    def test_register_user_with_valid_payload(self):
        response = self.client.post(reverse('register'),data=json.dumps(self.valid_payload) ,content_type=CONTENT_TYPE, follow=True)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    
    def test_register_user_with_invalid_payload(self):
        response = self.client.post(reverse('register'),data=json.dumps(self.invalid_payload) ,content_type=CONTENT_TYPE)
    ...
    def test_verify_email_with_valid_token(self):
        ...
        res = self.client.get('http://localhost:8000/auth/verify-email/?token='+token, content_type=CONTENT_TYPE)
    ...
    def test_user_update_its_user_profile_valid_payload(self):
        self.client.post(reverse('login'), data=json.dumps(self.valid_credentials), content_type=CONTENT_TYPE)
        response = self.client.put(reverse('user-profile'), data=json.dumps(self.valid_profile_payload) ,content_type=CONTENT_TYPE, secure=False, follow=True)
        self.assertEqual(response.status_code, status.HTTP_200_OK)    

    def test_user_update_its_user_profile_with_invalid_payload(self):
        self.client.post(reverse('login'), data=json.dumps(self.valid_credentials), content_type=CONTENT_TYPE)
        response = self.client.put(reverse('user-profile'), data=json.dumps(self.invalid_profile_payload) ,content_type=CONTENT_TYPE,  secure=False, follow=True)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST) 